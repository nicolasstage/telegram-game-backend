...
import { logger } from "../../../gateway/src/localServer/logger";
import { inspect } from "node:util";
import { RequestOptions, request } from "node:http";
import { createMessage, encrypt, enums, readKey, Key } from "openpgp";
import GuardianNodesV2ABI from "../../../gateway/src/localServer/CGPNv7New.json";
import { mapLimit } from "async";
import NodesInfo from "../../../gateway/src/localServer/CONET_nodeInfo.ABI.json";
...
const startGossip = (
  ...
  const kkk = request(option, (res) => {
    if (res.statusCode !== 200) {
      return logger(
        `startTestMiner got res.statusCode = [${res.statusCode}] != 200 error! restart`
      );
    }
    ...
    res.once("error", (err) => {
      ...
      logger(
        Colors.red(
          `startGossip [${node.ip_addr}] res on ERROR! Try to restart! `
        ),
        err.message
      );
    });
    ...
    res.once("end", () => {
      kkk.destroy();
      logger(
        Colors.red(`startGossip [${node.ip_addr}] res on END! Try to restart! `)
      );
    });
  });
  ...
  kkk.on("error", (err) => {
    setTimeout(() => {
      logger(
        Colors.red(
          `startGossip [${node.ip_addr}] requestHttps on Error! Try to restart! `
        ),
        err.message
      );
      startGossip(node, POST, callback);
...
const connectToGossipNode = async (node: nodeInfo) => {
  const key = Buffer.from(getRandomValues(new Uint8Array(16))).toString(
    "base64"
  );
  const command = {
    command: "mining",
  ...
  startGossip(node, JSON.stringify({ data: postData }), (err, _data) => {
    if (!_data) {
      return logger(
        Colors.magenta(
          `connectToGossipNode ${node.ip_addr} push ${_data} is null!`
        )
      );
    }
    ...
    try {
      ...
      if (!epochNode) {
        ...
        return logger(
          Colors.red(`${node.ip_addr} send unknow EPOCH ${data.epoch} data!`)
        );
      }
...
const moveData = () => {
  ...
  if (!obj) {
    return logger(
      Colors.red(`moveData Error! listenPool hasn't Epoch ${block} data! `)
    );
  }
  ...
  logger(
    Colors.magenta(
      `gossipStart sendEpoch ${block - 1} totalConnectNode ${
        previousGossipStatus.totalConnectNode
...
const startGossipListening = () => {
  if (!Guardian_Nodes.length) {
    return logger(
      Colors.red(`startGossipListening Error! gossipNodes is null!`)
    );
  }
  logger(
    Colors.blue(`startGossipListening gossipNodes = ${Guardian_Nodes.length}`)
  );
